...
def create_users(city_table):
    ...
    for i in range(100):
        ...
        domicile_city = city_table[random.randint(0, 14)]['city_id']
        users.append([user_id,name, phone_number, domicile_city])

    headers = ["user_id","name", "phone_number", "domicile"]
    # Use tabulate to create a table
    # table = tabulate(users, headers=headers)

    # Print the table to the console
    # print(table)
    create_csv("user",users,headers)
...
def create_advertisement(car_table):
    ...
    for i in range(100):
        ...
        car_product_id = car_table[random.randint(1, 49)]['product_id']
        user_id = random.randint(1, 100)
        is_allowed_bid=random.choice([True, False])
        title = text.rsplit()[0]
        description= text
        timestamp_ad=fake.date_time_between(start_date='-2y', end_date='now')
        advertisement.append([advertisement_id,car_product_id,user_id,is_allowed_bid,title,description,timestamp_ad])

    headers = ['advertisement_id','car_product_id','user_id','is_allowed_bid','title','description','timestamp_ad']
    # Use tabulate to create a table
...
def create_bid():
    ...
    for i in range(100):
        bid_id=i+1
        user_id = random.randint(1, 100)
        advertisement_id=random.randint(1, 100)
        ...
        bids.append([bid_id,user_id,advertisement_id,status_bid,timestamp_bid])

    headers = ['bid_id','user_id','advertisement_id','status_bid','timestamp_bid']
    # Use tabulate to create a table
